...
- Package manager: npm verified by `package.json` and `package-lock.json`.
...
- E2E script: `npm run test:e2e` maps to `playwright test` in `package.json`.
...
- AI/chat behavior: verified in `src/lib/runtime/agentRuntime.ts`, `src/hooks/useAgentController.ts`, `src/components/FloatingAgent.tsx`, `src/components/FullscreenAgent.tsx`, and `src/pages/GuidancePage.tsx`.
- Journaling/reflection features: verified in `src/pages/JournalPage.tsx`, `src/pages/DailyLightPage.tsx`, `src/lib/storage.ts`.
...
- Logging/monitoring/health checks: logging and safety events verified in `src/lib/storage.ts` and `src/lib/runtime/agentRuntime.ts`; health check in `src/workers/static-spa.ts`.
- Admin/content roles: verified in `src/pages/AdminPage.tsx`, `src/hooks/useAdminRole.ts`.
- End-to-End Visual Verification: Playwright configured via `tests/e2e/`.
...
- Dev dependencies include Vite, TypeScript, Vitest, Testing Library, ESLint, Playwright package, Tailwind, Wrangler, and Cloudflare worker types.
