...
async function checkRateLimit(
  ...
  // Upsert the rate limit record for today's window
  const { data, error } = await supabase
    .from("api_rate_limits")
    .upsert(
      {
        bucket_key: bucketKey,
        endpoint: "groq-guidance",
        request_count: 1,
  ...
  if (error) {
    // If we can't check the rate limit, allow but log
    console.warn("[groq-guidance] rate limit check failed:", error.message);
    return { allowed: true, remaining: limit };
  }

  // Increment if record already existed (upsert above inserted 1 for new records)
  if (data && data.request_count > 1) {
    // Record existed, we need to increment it
    const { data: updated } = await supabase
      .from("api_rate_limits")
      .update({ request_count: data.request_count + 1 })
      .eq("bucket_key", bucketKey)
      .eq("endpoint", "groq-guidance")
      .gte("window_start", windowStart.toISOString())
      .select("request_count")
      .single();

    const count = updated?.request_count ?? data.request_count + 1;
...
serve(async (req) => {
  ...
  try {
    ...
    const { allowed, remaining } = await checkRateLimit(supabase, bucketKey, rateLimit);
    if (!allowed) {
