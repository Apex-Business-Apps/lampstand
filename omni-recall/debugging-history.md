...
## 9. Persisted null Crashed the Installed PWA into the ErrorBoundary
- **Symptom**: The installed Android PWA opened to the ErrorBoundary fallback ("Something went wrong. An unexpected error occurred. Your data is safe.") on launch, while the same deployed bundle ran correctly in the desktop browser.
...
- **Why two earlier passes missed it**: PR #117 and PR #118 hardened individual call sites (`getSavedPassages`, `getCachedDaily`) and bumped the service worker cache, but left the boundary itself trusting `localStorage`.
- **Fix**: Made `src/lib/storage.ts` the single trusted read and write boundary. `get()` rejects a persisted null, falls back on shape drift, and completes partial legacy records from the defaults. `set()` no longer throws through a render when the store is full or disabled. `getProfile()` rejects a truthy non-record. `getPresenceScore()` repairs an unparseable timestamp instead of persisting `NaN`. `public/sw.js` v5 refreshes the cached offline shell on each successful navigation, since `install` only re-runs when that file changes.
