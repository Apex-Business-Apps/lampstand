...
## 11. Unhandled TypeError: Cannot Read Properties of Undefined (reading 'reference') in Daily Light & Resonance Engine
- **Symptom**: On navigating to `/app` (HomePage), visitors on web/PWA immediately crashed into the React ErrorBoundary with the console stack trace `TypeError: Cannot read properties of undefined (reading 'reference')` in `dailyLight.ts` / `ResonanceEngine.ts` / `HomePage.tsx`.
- **Root Cause**: In `src/lib/dailyLight.ts` line 30, `template.passage.reference === cached.passage.reference` evaluated unguarded when `cached` in local storage was malformed or missing the `passage` property. In `src/lib/resonance/ResonanceEngine.ts` line 392, `scoreCandidate` accessed `c.passage.reference` without optional chaining. Furthermore, `HomePage.tsx`, `DailyLightPage.tsx`, `KidsPage.tsx`, `LectioPage.tsx`, and `JournalPage.tsx` rendered `passage.reference` and `passage.text` without defensive fallbacks.
- **Fix**: 
  1. Guarded `getDailyLight` and `getDailyLightWithHistory` to validate `cached?.passage?.reference` and fallback to valid seed passages.
  2. Guarded `rankCandidates` to filter out candidates missing `passage` or `passage.reference` and guarded `scoreCandidate` to safely compute novelty penalties without crashing.
