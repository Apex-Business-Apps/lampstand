...
- **Note**: This entry's fix did not touch `get()`/`set()` in `storage.ts`, so it left a second, independent root cause for the identical symptom text in place. See entry 10.

## 10. Persisted null Crashed the Installed PWA into the ErrorBoundary
- **Symptom**: The same ErrorBoundary text as entry 9 ("Something went wrong. An unexpected error occurred. Your data is safe."), reproduced on a build that already included entry 9's fix. Founder screenshot showed this exact fallback with no chunk-loading error involved.
- **Root Cause**: `get<T>(key, fallback)` in `src/lib/storage.ts` returned `raw ? JSON.parse(raw) : fallback`. A persisted literal `"null"` is a truthy string, so it parsed to `null` and the declared default was skipped. Consumers dereference these getters immediately (`getPresenceScore().lastActivityAt` at the top of `HomePage`, `getKnowledge().lastStreakDate`, `getConsentState().localAdaptiveMemory`), so one corrupt key threw during render and unmounted the tree. The defect was data shaped, not platform shaped: the desktop profile held clean records, and the installed PWA is simply the surface that reads every record on first paint because `ProfileGuard` sends standalone traffic straight into `/app`.
- **Why three earlier passes missed it**: PR #117 hardened `getSavedPassages`/`getCachedDaily`. PR #118 bumped the service worker cache. PR #119 (entry 9) added per-field guards inside `getPresenceScore`/`incrementPresenceScore` only, for the `score` field specifically. None touched the `get()` boundary itself, so every other getter (`getKnowledge`, `getConsentState`, `getVoicePreferences`, `getSyncState`, `getAuthState`, `getPracticePreferences`) remained one corrupt key away from the same crash.
- **Fix**: Made `src/lib/storage.ts` the single trusted read and write boundary. `get()` rejects a persisted null, falls back on shape drift, and completes partial legacy records from the defaults. `set()` no longer throws through a render when the store is full or disabled. `getProfile()` rejects a truthy non-record. `getPresenceScore()` repairs an unparseable `lastActivityAt` and persists the repair, and keeps entry 9's `score` type guard since `get()`'s object-level merge does not validate individual field types. `public/sw.js` v6 refreshes the cached offline shell on each successful navigation, since `install` only re-runs when that file changes (entry 9 had already bumped this file to v5 for an unrelated reason: clearing stale chunk-hash caches).
- **Regression Shield**: `src/test/storage-corruption-boundary.test.ts` (nine corruption shapes against every getter, fails 16 tests against the previous implementation) and `tests/e2e/pwa-corrupt-storage.spec.ts` (five routes per corrupted key against the real UI, fails 4 specs against the previous implementation).
