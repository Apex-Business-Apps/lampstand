...
interface AgentPresenceProps {
  ...
  className?: string;
  size?: 'sm' | 'md' | 'lg';
  intensity?: number; // 0 to 1 for speaking amplitude
}
...
export function AgentPresence({ mode = 'idle', className = '', size = 'md', intensity = 0.5 }: AgentPresenceProps) {
  // Convert mode to animation classes
  const getAnimationClass = () => {
    switch (mode) {
      ...
      case 'speaking': return 'transition-all duration-150'; // Handled dynamically via style
